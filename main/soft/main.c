...
#define BUFFER      *((volatile unsigned int *) BUFFER_BASE)
...
#define IMAGES_NUMBER 8
  ...
  void video_processing_task() {
    for(;;) {
      ...
      BUFFER = (uint32_t)&images[processing_index%IMAGES_NUMBER];
      OUTPUT = (uint32_t) &images[(processing_index-1)%IMAGES_NUMBER];
      VIDEO_OUT = (uint32_t) &images[(processing_index-2)%IMAGES_NUMBER];
      my_printf("Processed index: %d\n\r",processing_index);
      processing_index++;
  ...
  int main(void) {
    my_printf("Beginning \r\n");
    computePolys(dataPoly);
    my_printf("Polys computed !\r\n");
    INCREMENT=dataPoly;
    irq_enable();
